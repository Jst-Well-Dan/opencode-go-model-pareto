...
.filter-panel{border:1.5px solid #0f766e;border-radius:12px;background:#fff;margin:12px 22px 0;box-shadow:0 4px 14px rgba(15,118,110,.12)}
.filter-summary{cursor:pointer;list-style:none;display:flex;gap:12px;align-items:center;padding:12px 16px 12px 0;font-size:.95rem}
.filter-summary::-webkit-details-marker{display:none}
.filter-summary .bar-accent{width:5px;align-self:stretch;border-radius:0 4px 4px 0;background:var(--teal);flex:none}
.filter-summary .bar-icon{width:30px;height:30px;border-radius:9px;background:#f0fdfa;border:1px solid #a7f3d0;display:grid;place-items:center;flex:none}
.filter-summary .bar-icon svg{width:16px;height:16px;stroke:#0f766e}
.filter-summary .bar-sub{color:var(--muted);font-size:.82rem}
.filter-summary .pill{font-size:.78rem;font-weight:700;border-radius:999px;padding:2px 11px;background:#f1f5f9;color:#475569;font-variant-numeric:tabular-nums}
.filter-summary .pill.alert{background:#fff7ed;color:#c2410c;border:1px solid #fed7aa}
.filter-summary .sum-chev{margin-left:auto;color:var(--teal);font-weight:800;font-size:1.05rem;transition:transform .15s}
.filter-panel[open] .sum-chev{transform:rotate(180deg)}
.filter-inner{padding:12px 16px 14px;border-top:1px solid #eef2f7}
.filter-inner .toolbar{display:flex;gap:8px;align-items:center;flex-wrap:wrap;margin-bottom:10px}
.filter-inner .toolbar .count{font-size:.82rem;color:var(--muted);margin-left:auto;font-variant-numeric:tabular-nums}
.filter-inner .search{font:inherit;padding:6px 12px;border:1px solid #cbd5e1;border-radius:8px;background:#fff}
.filter-inner .groups{display:grid;grid-template-columns:repeat(auto-fill,minmax(210px,1fr));gap:8px}
.filter-inner .groups details{border:1px solid #e2e8f0;border-radius:10px;background:#fff;padding:8px 10px}
.filter-inner .groups summary{cursor:pointer;font-weight:700;font-size:.86rem}
.filter-inner .groups label{display:flex;gap:8px;align-items:center;padding:4px 2px;font-size:.85rem;cursor:pointer;border-radius:6px}
.filter-inner .groups label:hover{background:#f1f5f9}
.filter-inner input[type=checkbox]{accent-color:var(--teal);width:16px;height:16px}
.filter-inner .brand-bulk{display:flex;gap:6px;margin:6px 0 4px}
.filter-inner .brand-bulk button{font-size:.75rem;padding:2px 8px}
.filter-summary:focus-visible,.filter-inner .groups summary:focus-visible{outline:3px solid #f59e0b;outline-offset:2px}
"""
...
<details class="filter-panel" id="filterPanel-oc"><summary class="filter-summary" id="filterSummary-oc"><span class="bar-accent"></span><span class="bar-icon"><svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round"><path d="M2 3h12l-4.5 5.2V12l-3 1.5V8.2L2 3z"/></svg></span><b>模型筛选</b><span class="bar-sub">按公司分组</span><span class="pill" id="filterCount-oc"></span><span class="sum-chev">▾</span></summary><div class="filter-inner"><div class="toolbar"><button id="filterAll-oc" type="button">全选</button><button id="filterReset-oc" type="button">重置</button><input class="search" id="filterSearch-oc" placeholder="搜索模型，如 muse…"><span class="count" id="filterCountInner-oc"></span></div><div class="groups" id="filterGroups-oc"></div></div></details>
...
function createParetoChart(prefix, baseData, quotaSnapshots, scoreSnapshots, yRanges, yTicksSnapshots, yMin, yMax, normalizationReference, yTicks, xConfig){
  ...
  function setParetoFlags(){plotted=data.filter(d=>d.intelligence!==null&&d.cost!==null&&!excluded.has(d.model));plotted.forEach(d=>{d.pareto=!plotted.some(o=>o!==d&&o.intelligence>=d.intelligence&&o.cost<=d.cost&&(o.intelligence>d.intelligence||o.cost<d.cost))});frontier=plotted.filter(d=>d.pareto).sort((a,b)=>a.cost-b.cost);}
  ...
  const BRAND_LABELS={"openai":"OpenAI","zhipu":"智谱","kimi":"Kimi","xiaomimimo":"小米 MiMo","minimax":"MiniMax","muse":"Muse","qwen":"Qwen","deepseek":"DeepSeek","hunyuan":"混元","longcat":"LongCat","grok":"Grok","gemini":"Gemini","inkling":"Inkling","nemotron":"Nemotron","step":"Step","ox":"Ox","unknown":"未知"};
  const excluded=new Set();let filterKeyword="";const filterBoxes=[],filterGroupSums=[];let filterGroupsEl=null;
  function brandLabel(b){return BRAND_LABELS[b]||b;}
  function updateFilterSummary(){const n=baseData.length-excluded.size;const c=document.getElementById("filterCount-"+prefix),c2=document.getElementById("filterCountInner-"+prefix);if(c){c.textContent=excluded.size?'已剔除 '+excluded.size+' 个 · '+n+'/'+baseData.length:'已选 '+n+'/'+baseData.length;c.classList.toggle("alert",excluded.size>0);}if(c2)c2.textContent='已选 '+n+'/'+baseData.length+' · 前沿 '+frontier.length+' 个';}
  function applyFilter(){filterBoxes.forEach(o=>{o.cb.checked=!excluded.has(o.m);});filterGroupSums.forEach(g=>{g.el.textContent=brandLabel(g.brand)+'（'+g.items.filter(m=>!excluded.has(m)).length+'/'+g.items.length+'）';});setParetoFlags();renderSummary();hideTip();render();}
  function buildFilterPanel(){
    filterGroupsEl=document.getElementById("filterGroups-"+prefix);
    if(!filterGroupsEl)return;
    const byBrand={};
    baseData.forEach(d=>{(byBrand[d.brand]=byBrand[d.brand]||[]).push(d.model);});
    Object.keys(byBrand).sort((a,b)=>brandLabel(a).localeCompare(brandLabel(b),"zh")).forEach(b=>{
      const items=byBrand[b];
      const det=document.createElement("details");det.dataset.brand=b;
      const s=document.createElement("summary");det.append(s);
      filterGroupSums.push({el:s,brand:b,items});
      const row=document.createElement("div");row.className="brand-bulk";
      const on=document.createElement("button");on.type="button";on.textContent="全选"+brandLabel(b);
      const off=document.createElement("button");off.type="button";off.textContent="清空"+brandLabel(b);
      on.addEventListener("click",()=>{items.forEach(m=>excluded.delete(m));applyFilter();});
      off.addEventListener("click",()=>{items.forEach(m=>excluded.add(m));applyFilter();});
      row.append(on,off);det.append(row);
      items.forEach(m=>{
        const lab=document.createElement("label");lab.dataset.model=m.toLowerCase();
        const cb=document.createElement("input");cb.type="checkbox";cb.checked=true;cb.setAttribute("aria-label",m);
        cb.addEventListener("change",()=>{cb.checked?excluded.delete(m):excluded.add(m);applyFilter();});
        lab.append(cb,document.createTextNode(m));det.append(lab);
        filterBoxes.push({cb,m});
      });
      filterGroupsEl.append(det);
    });
    const all=document.getElementById("filterAll-"+prefix),rs=document.getElementById("filterReset-"+prefix),si=document.getElementById("filterSearch-"+prefix);
    if(all)all.addEventListener("click",()=>{excluded.clear();applyFilter();});
    if(rs)rs.addEventListener("click",()=>{excluded.clear();applyFilter();});
    if(si)si.addEventListener("input",()=>{
      filterKeyword=si.value.trim().toLowerCase();
      filterGroupsEl.querySelectorAll("details").forEach(det=>{
        let vis=0;
        det.querySelectorAll("label").forEach(lab=>{const show=!filterKeyword||lab.dataset.model.includes(filterKeyword);lab.style.display=show?"":"none";if(show)vis++;});
        det.style.display=vis?"":"none";
      });
    });
    filterGroupSums.forEach(g=>{g.el.textContent=brandLabel(g.brand)+'（'+g.items.length+'/'+g.items.length+'）';});
    updateFilterSummary();
  }
  buildFilterPanel();
