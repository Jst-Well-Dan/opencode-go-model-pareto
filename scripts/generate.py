...
.filter-panel{border:1px solid #e2e8f0;border-radius:12px;background:#fbfcfe;margin:12px 22px 0}
.filter-panel[open]{box-shadow:0 4px 14px rgba(30,41,59,.06)}
.filter-summary{cursor:pointer;list-style:none;display:flex;gap:10px;align-items:center;padding:12px 16px;font-size:.9rem}
.filter-summary::-webkit-details-marker{display:none}
.filter-summary .sum-count{margin-left:auto;color:var(--muted);font-size:.82rem;font-variant-numeric:tabular-nums}
.filter-summary .sum-chev{color:var(--teal);font-weight:800;transition:transform .15s}
...
function createParetoChart(prefix, baseData, quotaSnapshots, scoreSnapshots, yRanges, yTicksSnapshots, yMin, yMax, normalizationReference, yTicks, xConfig){
  ...
  function updateFilterSummary(){const n=baseData.length-excluded.size;const c=document.getElementById("filterCount-"+prefix),c2=document.getElementById("filterCountInner-"+prefix);if(c)c.textContent='已选 '+n+'/'+baseData.length+(excluded.size?' · 已剔除 '+excluded.size+' 个':'');if(c2)c2.textContent='已选 '+n+'/'+baseData.length+' · 前沿 '+frontier.length+' 个';}
